"""Native cognis-connect emit for warmline — forward findings to any platform.

Maps warmline's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    warmline ... --format json | warmline-emit --to stix
    warmline-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('rule', '') + ' - ' + rec.get('lead', '')
        out['severity'] = rec.get('score', 'info')
        out['type'] = rec.get('rule', '')
